...
curl -s https://api.telegram.org/bot$token1/getUpdates | grep id |awk -F '"' '{print $9}' | tr -d : | tr -d , | jq >> idler5.txt
...
id=$(cat idler6.txt)
for i in $id ; do
	curl -s -X POST --silent -o /dev/null https://api.telegram.org/bot$token1/sendvideo -F chat_id=$i -F caption=$basliq -F video=@$konum
done
echo
echo
echo
